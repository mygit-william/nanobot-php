...
class ZhipuAdapter implements LLMInterface
{
    ...
    public function chat(array $messages, array $tools = []): string
    {
        try {
            ...
            if (empty($body)||!isset($body['choices'][0]['message']['content'])||empty($body['choices'][0]['message']['content'])||!$this->isStringValidJson($body['choices'][0]['message']['content'])) {
                    file_put_contents(__DIR__.'/../../storage/llm_error.log' , json_encode([
                    "messages" => $messages,
                    "response" => $body
                ...
                return json_encode(
                    [
                        "thought"=>"",
                        "reply"=>!empty($body['choices'][0]['message']['content'])&&!$this->isStringValidJson($body['choices'][0]['message']['content'])?$body['choices'][0]['message']['content']:"LLM 调用失败，未获取到响应,再次尝试调用接口，看看是否有更详细的错误信息",
                        'confidence'=>0.00
                    ]
                );
            }
            ...
            //     //容错，有时候接口会返回200但没有正确的内容
            //     file_put_contents(__DIR__.'/../../../storage/llm_error.log', FILE_APPEND , json_encode([
            //         "messages" => $messages,
            //         "response" => $body
            //     ]));
            ...
            //         [
            //             "thought"=>"",
            //             "reply"=>"LLM 调用失败，未获取到响应,再次尝试调用接口，看看是否有更详细的错误信息",
            //             'confidence'=>0.0
            //         ]
            //     );
       
                // return "LLM 调用失败，未获取到响应";
            // }
            return $body['choices'][0]['message']['content'] ?? 'AI 没有返回内容';
     ...
     */
    function sendChatRequest($url, $model, $apiKey, $messages)
    {
        // var_dump($apiKey);
        $payload = [
            "model" => $model,
